...
<div class="container" style="background: #222;border-radius: 12px;padding:15px;">
    <div class="col-md-12" >
        ...
        <div class="row" style="background: #ff66cc;padding: 15px">
                ...
                <table class="table table-striped" style="border:1px">
                    ...
                    <tr>
                        @if($code['coupon_condition'] == 2)
                            <td colspan="4" align="right">Tổng tiền thanh toán khi áp dụng mã giảm giá: {{number_format(($total-$code['coupon_number'] + $shipping_array['fee_ship']),0,',','.')}}vnđ</td>
                        @elseif($code['coupon_condition'] == 1)
                            <td colspan="4" align="right">Tổng tiền thanh toán khi  áp dụng mã giảm giá: {{number_format(( $total - ($total * $code['coupon_number'])/100)+ $shipping_array['fee_ship'],0,',','.')}}vnđ</td>
                        @else
            ...
            <p style="color:#fff;text-align: center;font-size: 15px;">Mọi chi tiết xin liên hệ website tại : <a target="_blank" href="http://project.net/">Shop</a>, hoặc liên hệ qua số hotline : 19005689.Xin cảm ơn quý khách đã đặt hàng shop chúng tôi.</p>
