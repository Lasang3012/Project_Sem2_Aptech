	...
	<div class="container" style="background: #222;border-radius: 12px;padding:15px;">
		<div class="col-md-12" >
			...
			<div class="row" style="background: cadetblue;padding: 15px">
						...
						<tbody>
							...
							<tr>
                              @if($code['coupon_condition'] == 2)
                                    <td colspan="4" align="right">Tổng tiền thanh toán khi áp dụng mã giảm giá: {{number_format(($total-$code['coupon_number'] + $shipping_array['fee']),0,',','.')}}vnđ</td>
                                  @elseif($code['coupon_condition'] == 1)
                                    <td colspan="4" align="right">Tổng tiền thanh toán khi  áp dụng mã giảm giá: {{number_format(( $total - ($total * $cou['coupon_number'])/100),0,',','.')}}vnđ</td>
                                @else
				...
				<p style="color:#fff">Mọi chi tiết xin liên hệ website tại : <a target="_blank" href="http://project.net/">Shop</a>, hoặc liên hệ qua số hotline : 19005689.Xin cảm ơn quý khách đã đặt hàng shop chúng tôi.</p>
